...
## 2026-08-01 — v0.2 distribution and verification

- **Plugin boundary:** Add a skills-only plugin manifest and no MCP server or app. Keep
  `.agents/skills/project-mentor` and `skills/project-mentor` byte-for-byte identical.
- **Local diagnostics:** Add a read-only `doctor` command whose report uses logical labels
  instead of absolute filesystem paths.
- **Anchor verification:** Verify only documented relative digest, symbol, configuration,
  and test locators. Refuse traversal and symlinks, report unsupported evidence as
  unavailable, and require `--write` plus an expected revision before mutation.
- **Machine contract:** Emit the existing receipt contract directly as deterministic JSON;
  retain Markdown as the default for compatibility.
- **Release gates:** Aggregate CI behind `CI Gate`, retain CodeQL independently, group only
  compatible Dependabot updates, and preserve unknown historical evaluation provenance as
