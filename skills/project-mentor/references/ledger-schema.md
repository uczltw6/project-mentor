...
| `evidence_marked_stale` | Legacy: `concept_id`, `evidence_id`, `reason`. Scoped: `owner_type`, `owner_id`, `evidence_id`, `reason` for a concept, decision, or milestone. |
...
The versioned receipt contract records `schema_version`, `generated_at`, `session_id`, `language`, `goal`, `mode`, selected milestone/concept/decision/deferred IDs, and `output_locator`. It can be emitted directly with `render --format json`; the default `markdown` format remains human-readable.
