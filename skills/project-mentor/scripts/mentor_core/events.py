...
def _apply_transition(ledger: dict[str, Any], event: dict[str, Any]) -> None:
    ...
    if event_type == "session_started":
        ...
        if "concept_id" in payload:
            owner = _by_id(ledger["concepts"], payload["concept_id"], "concept")
        else:
            collections = {
                "concept": "concepts",
                "decision": "decisions",
            ...
            owner = _by_id(ledger[collections[owner_type]], payload["owner_id"], owner_type)
        evidence = _by_id(owner["project_evidence"], payload["evidence_id"], "evidence")
