...
def validate_evidence(value: Any, path: str = "evidence", *, project: bool = True) -> None:
    ...
    elif evidence_class != "stale" and ("verifier" in evidence or "result" in evidence):
        raise InvalidInputError(
    ...
    if evidence_class == "stale":
        ...
        has_verification = "verifier" in evidence or "result" in evidence
        if has_verification and stale_from != "rule_verified":
            raise InvalidInputError(
                f"{path} stale verifier/result require stale_from=rule_verified"
            )
        if has_verification:
            if "verifier" not in evidence or "result" not in evidence:
                raise InvalidInputError(f"{path} stale verification requires both fields")
            _string(evidence["verifier"], f"{path}.verifier")
            _string(evidence["result"], f"{path}.result")
    ...
    if event_type == "session_started":
        ...
        if legacy:
            ...
            _exact_fields(
                payload,
                f"{path}.payload",
                {"owner_type", "owner_id", "evidence_id", "reason"},
            )
            _enum(
                payload["owner_type"],
                f"{path}.payload.owner_type",
                frozenset({"concept", "decision", "milestone"}),
            )
            _id(payload["owner_id"], f"{path}.payload.owner_id")
